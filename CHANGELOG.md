...
### Fixed
- Monitor no longer crashes with `TypeError: fromisoformat: argument must be
  str` when a usage quota bucket has no scheduled reset (`resets_at: null`,
  typically a bucket at 0% utilization). The bar now renders without the reset
  suffix in that case.
