...
def test_assistant_records_tool_calls() -> None:
    content = [{"type": "tool_use", "id": "t1", "name": "Read", "input": {}}]
    records = [assistant_rec(content=content)]
    result = list(assistant_records(records))
    assert result[0].usage.tools["Read"].calls == 1
    assert result[0].usage.tools["Read"].errors == 0
    assert result[0].agent_invocations == []


def test_assistant_records_tool_errors() -> None:
    content = [{"type": "tool_use", "id": "t1", "name": "Read", "input": {}}]
    records = [assistant_rec(content=content)]
    result = list(assistant_records(records, frozenset({"t1"})))
    assert result[0].usage.tools["Read"].errors == 1
...
def test_model_usage_tool_errors() -> None:
    ...
    msgs = list(assistant_records([assistant_rec(content=content)], tool_error_ids(user)))
    result = model_usage(msgs)
